...
using OxygenVK.AppSource.TabViewPages;

namespace OxygenVK.AppSource
{
	public sealed partial class TabViewWindowingSamplePage : UserControl
	{
		...
		private void SetupWindow(AppWindow window)
		{
			if(window == null)
			{
		...
		private async void Tabs_TabDroppedOutside(TabView sender, TabViewTabDroppedOutsideEventArgs args)
		{
			...
			TabViewWindowingSamplePage newPage = new TabViewWindowingSamplePage();
			newPage.SetupWindow(newWindow);

			ElementCompositionPreview.SetAppWindowContent(newWindow, new Page() { Content = newPage });
		...
		private void Tabs_TabDragStarting(TabView sender, TabViewTabDragStartingEventArgs args)
		{
			...
			TabViewItem firstItem = args.Tab;
		...
		{ 
			if(e.DataView.Properties.TryGetValue(DataIdentifier, out object obj))
			{
				...
				TabView destinationTabView = sender as TabView;
				System.Collections.Generic.IList<object> destinationItems = destinationTabView.TabItems;
				...
				if(destinationItems != null)
				{
					...
					{
						TabViewItem item = destinationTabView.ContainerFromIndex(i) as TabViewItem;
					...
					TabViewListView destinationTabViewListView = ((obj as TabViewItem).Parent as TabViewListView);
		...
		private void Tabs_AddTabButtonClick(TabView sender, object args) => sender.TabItems.Add(new TabViewItem()
		{
			IconSource = new Microsoft.UI.Xaml.Controls.SymbolIconSource()
			{
				Symbol = Symbol.Placeholder
		...
		private void Tabs_TabCloseRequested(TabView sender, TabViewTabCloseRequestedEventArgs args) => sender.TabItems.Remove(args.Tab);
	}
}
