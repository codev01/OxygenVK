...
namespace OxygenVK.AppSource.Views.Controls.Posts.ImageContainer
{
	...
	{
		public static void ProcessThumbnails(double maxW, double maxH, List<ContainerElement> thumbs, double marginBetween)
		{
			...
			if(!flag)
			{
				double num1 = doubleList1.Count > 0 ? Enumerable.Sum(doubleList1) / doubleList1.Count : 1;
				...
				else if(count == 2)
				{
					if(str == "ww" && num1 > 1.4 * num4 && doubleList1[1] - doubleList1[0] < 0.2)
					{
						...
						double height = Math.Min(width2 / doubleList1[0], Math.Min(width2 / doubleList1[1], (num3 - num2) / 2));
					...
					{
						double width2 = (width1 - margin) / doubleList1[1] / (1 / doubleList1[0] + 1 / doubleList1[1]);
				...
				else if(count == 4)
				{
					if(str == "wwww")
					{
						double width2 = width1;
						double height1 = Math.Min(width2 / doubleList1[0], (num3 - num2) * 0.66);
						double val2 = (width1 - 2 * margin) / (doubleList1[1] + doubleList1[2] + doubleList1[3]);
					...
					else
					{
						...
						double val2 = (num3 - 2 * num2) / (1 / doubleList1[1] + 1 / doubleList1[2] + 1 / doubleList1[3]);
				...
				else
				{
					...
					if(num1 > 1.1)
					{
						...
						{
							doubleList2.Add(Math.Max(1, val2));
						}
					}
					else
					{
					...
					foreach(string key in dictionary.Keys)
					{
						...
						double num6 = num2 * (doubleList3.Count - 1);
					...
					List<ContainerElement> thumbAttachmentList1 = new List<ContainerElement>(thumbs);
					...
					{
						List<ContainerElement> thumbAttachmentList2 = new List<ContainerElement>();
						...
						{
							ContainerElement thumbAttachment = thumbAttachmentList2[index2];
		...
		private static double CalculateMultiThumbsHeight(List<double> ratios, double width, double margin) =>
			(width - (ratios.Count - 1) * margin) / Enumerable.Sum(ratios);
